...
class MyMessageViewController: UIViewController,WKNavigationDelegate {

    let webview = WKWebView(frame: CGRect(x: 0, y: 0, width: kScreenWidth, height: kScreenHeight))
    override func viewDidLoad() {
        super.viewDidLoad()
        self.webview.navigationDelegate = self
        let url = URL(string: "http://dh.ecnucpp.cn:8885/message_center.html")!
    ...
    @objc func back() {
        if webview.url?.absoluteString == "http://dh.ecnucpp.cn:8885/message_center.html" {
