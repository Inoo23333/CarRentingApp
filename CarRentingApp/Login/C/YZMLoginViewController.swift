...
class YZMLoginViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let YZMLabel = UILabel()
        let YZMAttrString = NSMutableAttributedString(string: "发送验证码")
        YZMLabel.frame = CGRect(x: passwordView.frame.width-90-16, y: 13, width: 90, height: 22.5)
        YZMLabel.numberOfLines = 0
        let attr: [NSAttributedString.Key : Any] = [.font: UIFont.systemFont(ofSize: 16),.foregroundColor: UIColor(red: 0.81, green: 0.56, blue: 0, alpha: 1)]
        YZMAttrString.addAttributes(attr, range: NSRange(location: 0, length: YZMAttrString.length))
        YZMLabel.attributedText = YZMAttrString
        passwordView.addSubview(YZMLabel)
...
//        self.navigationController?.isNavigationBarHidden = false
    ...
    @objc func toMMLogin() {
        ...
        self.navigationController?.pushViewController(mmLoginVC, animated: true)
    }
