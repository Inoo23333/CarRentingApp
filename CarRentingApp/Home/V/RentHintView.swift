...
protocol RentHintViewDelegate:NSObjectProtocol {
    func toRentCar()
    func cancleReserve()
}
class RentHintView: UIView {
    weak var delegate:RentHintViewDelegate?
    override init(frame: CGRect) {
        ...
        let label = UILabel()
        let attrString = NSMutableAttributedString(string: "请在预约后15分钟内打开车门，否则因超时订单自动取消；首次开启开门系统开始计费；请在开门后5分钟内完成环境评测，提交测评报告有相应的积分奖励。")
